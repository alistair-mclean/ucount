...
#include "PreProcessor.h"
	...
	ImageProcessor::ImageProcessor(const cv::Mat & image, int clipLimit)
	{
	...
	ImageProcessor::ImageProcessor(const cv::Mat & image, const settings& pSettings)
	{
		_originalImage = image;
		cv::Mat blur;
	...
	std::vector<cv::Mat> ImageProcessor::SeparateChannels(const cv::Mat & image)
	{
	...
	std::vector<cv::Mat> ImageProcessor::GrayAndCLAHE(const std::vector<cv::Mat>& imageArray, int clipLimit)
	{
	...
	std::vector<cv::Mat> ImageProcessor::GrayAndCLAHE(const cv::Mat & image, int clipLimit)
	{
		cv::Mat gray = image;
	...
	void ImageProcessor::EdgeDetection(const cv::Mat & image)
	{
		// TODO: 
	...
	int ImageProcessor::CountObjects(const cv::Mat & image)
	{
		return 0;
	}
